...
$recycle_bin_path = realpath(__DIR__) . DIRECTORY_SEPARATOR . 'home' . DIRECTORY_SEPARATOR . 'recycle_bin' . DIRECTORY_SEPARATOR;
...
if (isset($_POST['selected'])) {
  ...
  if (isset($_POST['delete'])) {
    ...
    if (!in_array($delete_file, scandir($recycle_bin_path))) {
      rename($delete_file, $recycle_bin_path . basename($delete_file));
    }
    ...
    rename($_SESSION['cut_file'], $recycle_bin_path . basename($_SESSION['cut_file']));
    ...
    if (substr($_SESSION['copy_file'], 0, 1) === 'C') { // copy & paste
      ...
      rename($recycle_bin_path . basename(substr($_SESSION['cut_file'], 1)), $_SESSION['paste_path'] . DIRECTORY_SEPARATOR . basename(substr($_SESSION['cut_file'], 1)));
    }
  }
}
